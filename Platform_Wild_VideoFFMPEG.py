...
class Platform_Wild_VideoFFMPEG(PlatformBase):
    ...
    def initialize(self) -> bool:
        print("[Wild VideoFFMPEG] Initializing...")
    ...
    def run_frame(self, controls: dict[str, Any]) -> bool:
        if not self.is_initialized() or not self._last_rom_path:
            return False
        if controls:
            print("[Wild VideoFFMPEG] Note: Control mapping pending")
        return True
    ...
    def save_state(self) -> bytes:
        print("[Wild VideoFFMPEG] State save: Delegated to RetroArch")
        return b""

    def load_state(self, state_data: bytes) -> bool:
        print("[Wild VideoFFMPEG] State load: Delegated to RetroArch")
        return True
