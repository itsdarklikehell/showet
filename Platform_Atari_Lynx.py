...
class Platform_Atari_Lynx(PlatformBase):
    ...
    def run_frame(self, controls: Dict[str, Any]) -> bool:
        if not self.is_initialized() or not self._last_rom_path:
            return False
        if controls:
            print(f"[Atari Lynx] Note: Control mapping pending")
        return True
    ...
    def save_state(self) -> bytes:
        print(f"[Atari Lynx] State save: Delegated to RetroArch")
        return b""

    def load_state(self, state_data: bytes) -> bool:
        print(f"[Atari Lynx] State load: Delegated to RetroArch")
        return True
