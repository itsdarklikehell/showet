...
### Fixed: Boot Sequence Element Missing
**Issue:** `showet-boot-sequence.js` references element with id `loading` which didn't exist in the viewer HTML.

**Fix:** Added `<div id="loading" class="loading-screen">` element to the viewer HTML with appropriate styling.

### Fixed: Boot Sequence Script Loading
**Issue:** `showet-boot-sequence.js` was not included in the viewer HTML.

**Fix:** Added script tag to load the boot sequence module.

### Fixed: Duplicate NeoGeo Platform File
**Issue:** `Platform_neogeo.py` was generated by scaffold but `Platform_Snk_Neogeo.py` already exists as the official platform runner.

**Fix:** Removed the duplicate `Platform_neogeo.py` file.

### Fixed: Viewer Platform List Mismatch
**Issue:** Platform list in viewer used wrong naming convention (`nesfamicom` vs `nintendo_famicom`).
