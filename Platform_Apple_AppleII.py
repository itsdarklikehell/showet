...
class Platform_Apple_AppleII(PlatformBase):
    ...
    def initialize(self) -> bool:
        print("[Apple AppleII] Initializing...")
    ...
    def run_frame(self, controls: dict[str, Any]) -> bool:
        if not self.is_initialized() or not self._last_rom_path:
            return False
        if controls:
            print("[Apple AppleII] Note: Control mapping pending")
        return True
    ...
    def save_state(self) -> bytes:
        print("[Apple AppleII] State save: Delegated to RetroArch")
        return b""

    def load_state(self, state_data: bytes) -> bool:
        print("[Apple AppleII] State load: Delegated to RetroArch")
        return True
