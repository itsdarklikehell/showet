...
class Platform_Flash_Ruffle(PlatformBase):
    ...
    def initialize(self) -> bool:
        print("[Flash Ruffle] Initializing...")
    ...
    def run_frame(self, controls: dict[str, Any]) -> bool:
        if not self.is_initialized() or not self._last_rom_path:
            return False
        if controls:
            print("[Flash Ruffle] Note: Control mapping pending")
        return True
    ...
    def save_state(self) -> bytes:
        print("[Flash Ruffle] State save: Delegated to RetroArch")
        return b""

    def load_state(self, state_data: bytes) -> bool:
        print("[Flash Ruffle] State load: Delegated to RetroArch")
        return True
