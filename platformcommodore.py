...
class PlatformCommodore(PlatformCommon):
    def run(self):
        extensions = ['d64', 'd71', 'd80', 'd81', 'd82',
                      'g64', 'g41', 't64', 'tap', 'prg', 'p00', 'crt', 'bin', 'zip', 'gz', 'd6z', 'd7z', 'd8z', 'g4z', 'g6z', 'x6z', 'cmd', 'm3u', 'vfl', 'vsf', 'nib', 'nbz', 'd2m', 'd4m']
        # executable file types:
        d64s = self.find_files_with_extension(extensions[0])
        d71s = self.find_files_with_extension(extensions[1])
        d80s = self.find_files_with_extension(extensions[2])
        d81s = self.find_files_with_extension(extensions[3])
        d82s = self.find_files_with_extension(extensions[4])
        g64s = self.find_files_with_extension(extensions[5])
        g41s = self.find_files_with_extension(extensions[6])
        x64s = self.find_files_with_extension(extensions[7])
        t64s = self.find_files_with_extension(extensions[8])
        taps = self.find_files_with_extension(extensions[9])
        prgs = self.find_files_with_extension(extensions[10])
        p00s = self.find_files_with_extension(extensions[11])
        crts = self.find_files_with_extension(extensions[12])
        bins = self.find_files_with_extension(extensions[13])
        zips = self.find_files_with_extension(extensions[14])
        gzs = self.find_files_with_extension(extensions[15])
        d6zs = self.find_files_with_extension(extensions[16])
        d7zs = self.find_files_with_extension(extensions[17])
        d8zs = self.find_files_with_extension(extensions[18])
        g4zs = self.find_files_with_extension(extensions[19])
        g6zs = self.find_files_with_extension(extensions[20])
        x6zs = self.find_files_with_extension(extensions[21])
        cmds = self.find_files_with_extension(extensions[22])
        m3us = self.find_files_with_extension(extensions[23])
        vfls = self.find_files_with_extension(extensions[24])
        vsfs = self.find_files_with_extension(extensions[25])
        nibs = self.find_files_with_extension(extensions[26])
        nbzs = self.find_files_with_extension(extensions[27])
        d2ms = self.find_files_with_extension(extensions[28])
        d4ms = self.find_files_with_extension(extensions[29])
