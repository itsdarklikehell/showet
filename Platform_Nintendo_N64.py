...
class Platform_Nintendo_N64(PlatformBase):
    ...
    def initialize(self) -> bool:
        print("[Nintendo N64] Initializing...")
    ...
    def run_frame(self, controls: dict[str, Any]) -> bool:
        if not self.is_initialized() or not self._last_rom_path:
            return False
        if controls:
            print("[Nintendo N64] Note: Control mapping pending")
        return True
    ...
    def save_state(self) -> bytes:
        print("[Nintendo N64] State save: Delegated to RetroArch")
        return b""

    def load_state(self, state_data: bytes) -> bool:
        print("[Nintendo N64] State load: Delegated to RetroArch")
        return True
