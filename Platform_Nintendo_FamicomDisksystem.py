...
class Platform_Nintendo_FamicomDisksystem(PlatformBase):
    ...
    def initialize(self) -> bool:
        print("[Nintendo FamicomDisksystem] Initializing...")
    ...
    def run_frame(self, controls: dict[str, Any]) -> bool:
        if not self.is_initialized() or not self._last_rom_path:
            return False
        if controls:
            print("[Nintendo FamicomDisksystem] Note: Control mapping pending")
        return True
    ...
    def save_state(self) -> bytes:
        print("[Nintendo FamicomDisksystem] State save: Delegated to RetroArch")
        return b""

    def load_state(self, state_data: bytes) -> bool:
        print("[Nintendo FamicomDisksystem] State load: Delegated to RetroArch")
        return True
