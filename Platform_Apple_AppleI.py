...
class Platform_Apple_AppleI(PlatformBase):
    ...
    def initialize(self) -> bool:
        print("[Apple AppleI] Initializing...")
    ...
    def run_frame(self, controls: dict[str, Any]) -> bool:
        if not self.is_initialized() or not self._last_rom_path:
            return False
        if controls:
            print("[Apple AppleI] Note: Control mapping pending")
        return True
    ...
    def save_state(self) -> bytes:
        print("[Apple AppleI] State save: Delegated to RetroArch")
        return b""

    def load_state(self, state_data: bytes) -> bool:
        print("[Apple AppleI] State load: Delegated to RetroArch")
        return True
