...
class Platform_Raspberry_Pi(PlatformBase):
    ...
    def initialize(self) -> bool:
        print("[Raspberry Pi] Initializing...")
    ...
    def run_frame(self, controls: dict[str, Any]) -> bool:
        if not self.is_initialized() or not self._last_rom_path:
            return False
        if controls:
            print("[Raspberry Pi] Note: Control mapping pending")
        return True
    ...
    def save_state(self) -> bytes:
        print("[Raspberry Pi] State save: Delegated to RetroArch")
        return b""

    def load_state(self, state_data: bytes) -> bool:
        print("[Raspberry Pi] State load: Delegated to RetroArch")
        return True
