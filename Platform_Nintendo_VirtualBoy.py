...
class Platform_Nintendo_VirtualBoy(PlatformBase):
    ...
    def initialize(self) -> bool:
        print("[Virtual Boy] Initializing...")
    ...
    def run_frame(self, controls: dict[str, Any]) -> bool:
        if not self.is_initialized() or not self._last_rom_path:
            return False
        if controls:
            print("[Virtual Boy] Note: Control mapping pending")
        return True
    ...
    def save_state(self) -> bytes:
        print("[Virtual Boy] State save: Delegated to RetroArch")
        return b""

    def load_state(self, state_data: bytes) -> bool:
        print("[Virtual Boy] State load: Delegated to RetroArch")
        return True