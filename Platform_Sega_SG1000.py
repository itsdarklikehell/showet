...
class Platform_Sega_SG1000(PlatformBase):
    ...
    def initialize(self) -> bool:
        print("[Sega SG1000] Initializing...")
    ...
    def run_frame(self, controls: dict[str, Any]) -> bool:
        if not self.is_initialized() or not self._last_rom_path:
            return False
        if controls:
            print("[Sega SG1000] Note: Control mapping pending")
        return True
    ...
    def save_state(self) -> bytes:
        print("[Sega SG1000] State save: Delegated to RetroArch")
        return b""

    def load_state(self, state_data: bytes) -> bool:
        print("[Sega SG1000] State load: Delegated to RetroArch")
        return True
