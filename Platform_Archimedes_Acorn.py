...
class Platform_Archimedes_Acorn(PlatformBase):
    ...
    def run_frame(self, controls: Dict[str, Any]) -> bool:
        if not self.is_initialized() or not self._last_rom_path:
            return False
        if controls:
            print(f"[Archimedes Acorn] Note: Control mapping pending")
        return True
    ...
    def save_state(self) -> bytes:
        print(f"[Archimedes Acorn] State save: Delegated to RetroArch")
        return b""

    def load_state(self, state_data: bytes) -> bool:
        print(f"[Archimedes Acorn] State load: Delegated to RetroArch")
        return True
