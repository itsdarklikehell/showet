...
class Platform_Atari_5200(PlatformBase):
    ...
    def run_frame(self, controls: Dict[str, Any]) -> bool:
        if not self.is_initialized() or not self._last_rom_path:
            return False
        if controls:
            print(f"[Atari 5200] Note: Control mapping pending")
        return True
    ...
    def save_state(self) -> bytes:
        print(f"[Atari 5200] State save: Delegated to RetroArch")
        return b""

    def load_state(self, state_data: bytes) -> bool:
        print(f"[Atari 5200] State load: Delegated to RetroArch")
        return True
