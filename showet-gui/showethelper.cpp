...
ShowetHelper : ShowetHelper(QObject *parent) : QObject(parent), m_running(false)
{
    connect(&m_showetProcess, SIGNAL(finished(int, QProcess
                                              : ExitStatus)),
            this, SLOT(processFinished(int, QProcess
                                       : ExitStatus)));
    connect(this, SIGNAL(runError(QString)),
            this, SLOT(printRunError(QString)));
#if QT_VERSION < QT_VERSION_CHECK(5, 6, 0)
    connect(&showetProcess, SIGNAL(errorOccurred(QProcess
                                                 : ProcessError)),
            this, SLOT(processErrorOccurred(QProcess
                                            : ProcessError)));
#endif
}
...
void ShowetHelper : processFinished(int exitCode, QProcess
...
void ShowetHelper : processErrorOccurred(QProcess
                                         : ProcessError error)
{
    Q_UNUSED(error);
    QString err = m_showetProcess.readAll();
